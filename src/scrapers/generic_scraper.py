...
class GenericScraper(BaseScraper):
    ...
    def _find_rss_feed(self) -> Optional[str]:
    ...
    def _scrape_article_urls_from_html(self) -> list[str]:
        ...
        for link in soup.find_all("a", href=True):
            ...
            if re.search(r"/\d{4}/\d{2}/", path) or len(path.split("/")) >= 2:
                if full_url not in urls and full_url != self.blog_url:
    ...
    def _get_rss_entry(self, url: str) -> Optional[dict]:
    ...
    def _extract_title(self, soup: BeautifulSoup, rss_entry: Optional[dict]) -> Optional[str]:
    ...
    def _extract_content(self, soup: BeautifulSoup, rss_entry: Optional[dict]) -> str:
    ...
    def _extract_date(self, soup: BeautifulSoup, rss_entry: Optional[dict]):
    ...
    def _extract_author(self, soup: BeautifulSoup, rss_entry: Optional[dict]) -> Optional[str]:
        """Extract author name."""
        # Try RSS
        if rss_entry:
    ...
    def _extract_tags(self, soup: BeautifulSoup, rss_entry: Optional[dict]) -> list[str]:
